/***************************************************************************
  tag: Ruben Smits  Tue Nov 16 09:21:07 CET 2010  HelloComponent.cpp

                        HelloComponent.cpp -  description
                           -------------------
...
class HelloComponent : public RTT::TaskContext{
private:  
  ...
  InputPort<std_msgs::Int32> myIntPort;
...
public:
  HelloComponent(const std::string& name):
    TaskContext(name),
    inport("float_in"),outport("float_out"),
    sinport("string_in"),soutport("string_out","Hello Robot"),
    myIntPort("myInt_in"),
    prop_answer("Connection Test"),prop_counter_step(0.01),prop_service_call_counter(0.0),
    updated("updated")
  {
    ...
    this->addPort(myIntPort).doc("Sends MyIntForTests");
    ...
    this->addProperty("myInt", myInt).doc("Int for test.");

    this->provides()->addOperation("increment",&HelloComponent::increment,this,RTT::OwnThread);
    this->provides()->addOperation("getMyInt",&HelloComponent::getMyInt,this,RTT::OwnThread);
    this->provides()->addOperation("incrementMyInt",&HelloComponent::incrementMyInt,this,RTT::OwnThread);
    this->provides()->addOperation("reciveMyInt",&HelloComponent::reciveMyInt,this,RTT::OwnThread);
...
private:
  ...
  void reciveMyInt(){
    std_msgs::Int32 mydata;
    if(NewData==myIntPort.read(mydata)){
      myInt=mydata.data;
    }
  }

};
ORO_CREATE_COMPONENT(HelloComponent)
